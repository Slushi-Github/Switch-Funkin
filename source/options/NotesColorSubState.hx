...
class NotesColorSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float) {
		...
		{
			//SlDebug.log('changed controller mode');
			FlxG.mouse.visible = !controls.controllerMode;
			controllerPointer.visible = controls.controllerMode;
		...
		if(hexTypeNum > -1)
		{
			...
			{
				//SlDebug.log('keyPressed: $keyPressed, lil str: ' + allowedTypeKeys.get(keyPressed));
		...
		if(pointerOverlaps(copyButton))
		{
			...
			if(generalPressed)
			{
				...
				FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
				SlDebug.log('copied: ' + Clipboard.text);
			}
			hexTypeNum = -1;
		}
		else if (pointerOverlaps(pasteButton))
		{
			...
			if(generalPressed)
			{
				...
				//SlDebug.log('#${Clipboard.text.trim().toUpperCase()}');
