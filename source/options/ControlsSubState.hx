...
class ControlsSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		var text:Alphabet = new Alphabet(60, 90, 'CTRL\nLEFT OR RIGHT SHOULDER', false);
	...
	override function update(elapsed:Float)
	{
		...
		if(!binding)
		{
			if(FlxG.keys.justPressed.ESCAPE || FlxG.gamepads.anyJustPressed(#if switch A #else B #end))
			{
				close();
				return;
			}
			...
			if(FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(#if switch B #else A #end))
			{
