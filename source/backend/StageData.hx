...
class StageData
{
	...
	public static function loadDirectory(SONG:SwagSong)
	{
		...
		if (stageFile != null)
		{
			...
			// IMPORTANTE: Esto le dice a Paths dónde buscar primero (ej: 'week7')
	...
	public static function getStageFile(stage:String):StageFile
	{
		try
		{
			...
			if (FileSystem.exists(fsPath))
				return cast tjson.TJSON.parse(File.getContent(fsPath));
			#elseif MODS_ALLOWED
		...
		{
			trace('Error loading stage file ($stage): ' + e.message);
		}
		return dummy();
	}
			...
			?ignoreFilters:Bool = false)
	{
		...
		{
			if (addedObjects.exists(data.name))
				continue;
	...
	public static function validateVisibility(filters:Int):Bool
	{
		...
		if ((filters & FREEPLAY) == FREEPLAY)
			if (PlayState.isStoryMode)
				return false;

		if (ClientPrefs.data.lowQuality)
			return (filters & LOW_QUALITY) == LOW_QUALITY;

		return (filters & HIGH_QUALITY) == HIGH_QUALITY;
	}
}