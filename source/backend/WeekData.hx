...
class WeekData
{
	...
	for (i in 0...directories.length)
	{
		...
		#if switch
		var fsPath = Paths.toFileSystemPath(directory);
		...
		if (FileSystem.exists(directory))
		{
			...
			#if switch
			for (file in FileSystem.readDirectory(fsPath))
			#else
			...
			#end
			{
				...
				#if switch
				var fsCheckPath = Paths.toFileSystemPath(path);
				if (!FileSystem.isDirectory(fsCheckPath) && file.endsWith('.json'))
				#else
	...
	private static function getWeekFile(path:String):WeekFile
	{
		...
		if (FileSystem.exists(fsPath))
		{
			rawJson = File.getContent(fsPath);
		}
		#else
		if (FileSystem.exists(path))
		{
