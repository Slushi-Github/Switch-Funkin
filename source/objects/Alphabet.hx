...
class AlphaCharacter extends FlxSprite
{
	...
	// Ejemplo de cómo debe quedar loadAlphabetData para funcionar correctamente en Switch
	...
	public static function loadAlphabetData(request:String = 'alphabet')
	{
		...
		#if switch
		if (!Paths.fileExists('images/$request.json', TEXT))
		#elseif MODS_ALLOWED
		...
		try
		{
			// Usar Paths.getTextFromFile que maneja todo correctamente
			var jsonText:String = Paths.getTextFromFile('images/${request == path ? "alphabet" : request}.json');
			if (jsonText == null)
				jsonText = Paths.getTextFromFile('images/alphabet.json');
			...
			SlDebug.log('Reloaded letters successfully ($path)!');
		}
		catch (e:Dynamic)
		{
			FlxG.log.error('Error on loading alphabet data: $e');
			SlDebug.log('Error on loading alphabet data: $e');
		}
