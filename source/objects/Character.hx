...
class Character extends FlxSprite
{
	...
	**/
	...
	public function changeCharacter(character:String)
	{
		...
		// 1. Intentamos obtener el contenido del JSON directamente con el helper
		var rawJson:String = Paths.getTextFromFile(characterPath);

		// 2. Si no existe o falló, cargamos el personaje por defecto
		if (rawJson == null)
		{
			SlDebug.log('Character "$character" not found, using default character', DEBUG);
		...
		try
		{
			SlDebug.log('Loading character file: $characterPath', DEBUG);
			// 3. Parseamos el JSON que ya obtuvimos (sea el original o el default)
			loadCharacterFile(Json.parse(rawJson));
		}
		catch (e:Dynamic)
		{
			SlDebug.log('Error loading character file of "$character": $e', ERROR);
		}
	...
	public function loadCharacterFile(json:Dynamic)
	{
		...
		if (#if MODS_ALLOWED FileSystem.exists(#if switch Paths.toFileSystemPath(animToFind) #else animToFind #end) || #end Assets.exists(animToFind))
			isAnimateAtlas = true;
		#end
		...
		#end
		SlDebug.log('Loaded file to character ' + curCharacter);
	}
