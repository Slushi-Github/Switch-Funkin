	...
	public function playAnim(animName:String = null, playIdle:Bool = false) {
		...
		if(dialogueAnimations.exists(leAnim)) {
			...
			if(playIdle) {
				...
				//SlDebug.log('Setting idle offsets: ' + anim.idle_offsets);
			} else {
				offset.set(anim.loop_offsets[0], anim.loop_offsets[1]);
				//SlDebug.log('Setting loop offsets: ' + anim.loop_offsets);
			}
		} else {
			offset.set(0, 0);
			SlDebug.log('Offsets not found! Dialogue character is badly formatted, anim: ' + leAnim + ', ' + (playIdle ? 'idle anim' : 'loop anim'));
		}
	}
