...
import slushi.slushiLua.SwitchLua;
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		//SlDebug.log('Lua version: ' + Lua.version());
		//SlDebug.log("LuaJIT version: " + Lua.versionJIT());
		...
		Lua_helper.add_callback(lua, "removeLuaScript", function(luaFile:String) {
			...
			if(luaPath != null)
			{
				...
				for (luaInstance in game.luaArray)
				{
					if(luaInstance.scriptName == luaPath)
					{
						SlDebug.log('Closing lua script $luaPath');
						luaInstance.stop();
						foundAny = true;
					}
				}
		...
		Lua_helper.add_callback(lua, "removeHScript", function(scriptFile:String) {
			...
			if(scriptPath != null)
			{
				...
				for (script in game.hscriptArray)
				{
					if(script.origin == scriptPath)
					{
						SlDebug.log('Closing hscript $scriptPath');
						script.destroy();
						foundAny = true;
					}
				}
		...
		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			...
			variables.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer)
			{
				...
				//SlDebug.log('Timer Completed: ' + tag);
			}, loops));
			return tag;
		});
		...
		Lua_helper.add_callback(lua, "triggerEvent", function(name:String, ?value1:String = '', ?value2:String = '') {
			...
			//SlDebug.log('Triggered event: ' + name + ', ' + value1 + ', ' + value2);
			return true;
		});
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, ?music:String = null) {
			...
			#if switch
			if (!Paths.fileExists('data/$songPath/${dialogueFile}_${ClientPrefs.data.language}.json', TEXT))
			#elseif MODS_ALLOWED
			...
			#if switch
			if (!Paths.fileExists('data/$songPath/$dialogueFile.json', TEXT))
			#elseif MODS_ALLOWED
			if(FileSystem.exists(path))
			#else
			if(Assets.exists(path, TEXT))
		...
		addLocalCallback("close", function() {
			closed = true;
			SlDebug.log('Closing script $scriptName');
			return closed;
		});
		...
		try{
			...
			if(resultStr != null && result != 0) {
				final realScriptName:String = scriptName != null || !isString || scriptName != '' ? scriptName : 'Unknown';
				SlDebug.log('Error loading Lua script: [$realScriptName]: $resultStr', ERROR);
				#if windows
				lime.app.Application.current.window.alert('Error loading Lua script: [$realScriptName]:\n$resultStr', 'Error on Lua script!');
				#elseif switch
				SwitchUtils.showError(0, 'Error loading Lua script: [$realScriptName]:\n$resultStr');
				#else
		...
		SlDebug.log('lua file loaded succesfully: ' + scriptName);
