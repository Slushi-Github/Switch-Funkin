...
class TankmenBG extends FlxSprite
{
	...
	public function new(x:Float, y:Float, facingRight:Bool)
	{
		...
		// 1. Especificamos 'week7' (o la carpeta donde esté el asset)
		// 2. Guardamos el resultado en una variable para verificarlo
		frames = Paths.getSparrowAtlas('tankmanKilled1', 'week7');

		// SEGURIDAD: Si no se encuentran los frames, detenemos la carga de este objeto
		if (frames == null)
		{
			SlDebug.log('TankmanKilled1 no encontrado en week7 ni shared. Ocultando objeto.', ERROR);
			visible = false;
			active = false;
			return;
		}
