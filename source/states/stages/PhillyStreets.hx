...
class PhillyStreets extends BaseStage
{
	...
	function videoCutscene(?videoName:String = null)
	{
		...
		{
			#if VIDEOS_ALLOWED
			var video = game.startVideo(videoName);

			// Check if videoCutscene was created successfully
			if (video != null)
			{
			...
			{
				// Video failed to load, skip to cutscene
				SlDebug.log('Video failed to load: $videoName', WARNING);
				videoEnded = true;
				videoCutscene();
			}
