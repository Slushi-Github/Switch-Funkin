...
class PlayState extends MusicBeatState
{
	...
	{
		//SlDebug.log('Playback Rate: ' + playbackRate);
		...
		if (ClientPrefs.data.enableHitbox) {
			addHitbox();
			hitbox.visible = true;
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true)
	{
		...
		if (foundFile)
		{
			...
			SlDebug.log("Video not found: " + fileName, ERROR);
		}
		#end
		#else
	...
	function startSong():Void
	{
		...
		if(paused) {
			//SlDebug.log('Oopsie doopsie! Paused sound');
			FlxG.sound.music.pause();
			vocals.pause();
			opponentVocals.pause();
		}
	...
	private function generateSong():Void
	{
		...
		SlDebug.log('["${SONG.song.toUpperCase()}" CHART INFO]: Ghost Notes Cleared: $ghostNotesCaught');
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		if (ClientPrefs.data.enableHitbox) {
			...
			if (isHitboxArrowMode && ClientPrefs.data.downScroll)
				playerStrumLineY -= 50;
	...
	function resyncVocals():Void
	{
		...
		SlDebug.log('resynced vocals at ' + Math.floor(Conductor.songPosition));
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Play Animation':
				//SlDebug.log('Anim to play: ' + value1);
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					SlDebug.log('LOADING NEXT SONG');
					SlDebug.log(Paths.formatToSongPath(PlayState.storyPlaylist[0]) + difficulty);
			...
			{
				SlDebug.log('WENT BACK TO FREEPLAY??');
				Mods.loadTopMod();
				#if DISCORD_ALLOWED DiscordClient.resetClientID(); #end
	...
	private function keysCheck():Void
	{
		...
		{
			var held:Bool = controls.pressed(key);
			var pressed:Bool = controls.justPressed(key);
			var released:Bool = controls.justReleased(key);

			#if TOUCH_CONTROLS_ALLOWED
			if (ClientPrefs.data.enableHitbox && hitbox != null && hitbox.visible)
			{
				var touchButton:TouchButton = switch (key)
				{
					case 'note_left': hitbox.buttonLeft;
					case 'note_down': hitbox.buttonDown;
					case 'note_up': hitbox.buttonUp;
					case 'note_right': hitbox.buttonRight;
					default: null;
				}

				if (touchButton != null)
				{
					held = held || touchButton.pressed;
					pressed = pressed || touchButton.justPressed;
					released = released || touchButton.justReleased;
				}
			}
			#end
	...
	override function beatHit()
	{
		if(lastBeatHit >= curBeat) {
			//SlDebug.log('BEAT HIT: ' + curBeat + ', LAST HIT: ' + lastBeatHit);
			return;
		}
	...
	public function initHScript(file:String)
	{
		...
		try
		{
			...
			SlDebug.log('initialized hscript interp successfully: $file');
	...
	public function RecalculateRating(badHit:Bool = false, scoreBop:Bool = true) {
		...
		if(ret != LuaUtils.Function_Stop)
		{
			...
			if(totalPlayed != 0) //Prevent divide by 0
			{
				...
				//SlDebug.log((totalNotesHit / totalPlayed) + ', Total: ' + totalPlayed + ', notes hit: ' + totalNotesHit);
