...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		/*buttonModFolder = new MenuButton(buttonX, myY, buttonWidth, buttonHeight, "MODS FOLDER", function() {
			...
			{
				SlDebug.log('created missing folder');
	...
	override function update(elapsed:Float)
	{
		if(controls.BACK && hoveringOnMods)
		{
			...
			if(waitingToRestart)
			{
				...
				SwitchTitleState.initialized = false;
				SwitchTitleState.closedState = false;
				FlxG.sound.music.fadeOut(0.3);
				if (SlushiFreeplayState.vocals != null)
				{
					SlushiFreeplayState.vocals.fadeOut(0.3);
					SlushiFreeplayState.vocals = null;
				}
		...
		else
		{
			...
			if(nextAttempt < 0)
			{
				...
				if(modsList.all.length > 0)
				{
					SlDebug.log('mod(s) found! reloading');
					reload();
				}
			}
		}
	...
	function updateItemPositions()
	{
		...
		for (i => mod in modsGroup.members)
		{
			if(mod == null)
			{
				SlDebug.log('Mod #$i is null, maybe it was ' + modsList.all[i]);
				continue;
			}
	...
	function moveModToPosition(?mod:String = null, position:Int = 0)
	{
		...
		SlDebug.log('Moved mod $mod to position $position');
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		if(FileSystem.exists(path))
		{
			try
			{
				//SlDebug.log('trying to load settings: $folder');
			...
			catch(e:Dynamic)
			{
				...
				#end
				SlDebug.log('$errorTitle - $errorMsg');
			}
		}
