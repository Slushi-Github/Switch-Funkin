...
class LoadingState extends MusicBeatState
{
			...
			if(FileSystem.exists(scriptPath))
			{
				try
				{
					...
					if(hscript.exists('onCreate'))
					{
						...
						SlDebug.log('initialized hscript interp successfully: $scriptPath');
						return super.create();
					}
					else
					{
						SlDebug.log('"$scriptPath" contains no \"onCreate" function, stopping script.');
					}
				}
				catch(e:IrisError)
				{
	...
	public static function checkLoaded():Bool
	{
		...
		{
			if (bitmap != null && Paths.cacheBitmap(originalBitmapKeys.get(key), bitmap) != null) {} //SlDebug.log('finished preloading image $key');
			else SlDebug.log('failed to cache image $key');
		}
		requestedBitmaps.clear();
		originalBitmapKeys.clear();
		// SlDebug.log('we checked if loaded');
	...
	public static function loadNextDirectory()
	{
		...
		SlDebug.log('Setting asset folder to ' + directory);
	}
	...
	public static function prepareToSong()
	{
		...
		.onError((err:Dynamic) -> {
			SlDebug.log('ERROR! while preparing song: $err');
		});
	}
	...
	static function clearInvalidFrom(arr:Array<String>, prefix:String, ext:String, type:AssetType, ?parentFolder:String = null)
	{
		for (folder in arr.copy())
		{
			...
			if(nam.endsWith('/'))
			{
				...
				//SlDebug.log('Folder detected! ' + folder);
			}
		}
		...
		while(i < arr.length)
		{
			...
			//SlDebug.log('attempting on $prefix: $myKey');
			var doTrace:Bool = false;
			if(member.endsWith('/') || (!Paths.fileExists(myKey, type, false, parentFolder) && (doTrace = true)))
			{
				arr.remove(member);
				if(doTrace) SlDebug.log('Removed invalid $prefix: $member');
			}
			else i++;
		}
	}
	...
	{
		// SlDebug.log('scheduled $func in threadPool');
		#if debug
		...
		threadPool.run(() -> {
			...
			SlDebug.log('$traceData took ${threadStart - threadSchedule}s to start preloading');
			#end
			...
			try {
				if (func() != null) {
					...
					SlDebug.log('finished preloading $traceData in ${diff}s');
					#end
				} else SlDebug.log('ERROR! fail on preloading $traceData', ERROR);
			}
			catch(e:Dynamic) {
				SlDebug.log('ERROR! fail on preloading $traceData: $e', ERROR);
			}
			// mutex.acquire();
			loaded++;
			// mutex.release();
	...
	inline private static function preloadCharacter(char:String, ?prefixVocals:String)
	{
		...
		{
			SlDebug.log('Fail on preloading character $char: ${e.details()}', ERROR);
		}
	}
